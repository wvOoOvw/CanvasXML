...
const useAnimationDestinationRateTimeWithObject = (option) => {
  return Object.keys(option.object).reduce((r, i) => {
    if (typeof option.object[i] === 'number') {
      return {
        ...r,
        [i]: useAnimationDestinationRateTime({ defaultCount: option.object[i], defaultDestination: option.object[i], ...option })
      }
    }
    if (typeof option.object[i] === 'object') {
      return {
        ...r,
        [i]: useAnimationDestinationRateTimeWithObject({ ...option, object: option.object[i] })
      }
    }
  }, Object())
}
