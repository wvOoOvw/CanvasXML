...
const App = (props) => {
  return new Array(Math.ceil(Position.vmax(props.position) * 100 / props.gap / 2)).fill().map((i, index) => {
    if (index === 0) {
      return <>
        <rect save fill x={props.position.x} y={props.position.cy} w={Position.vmax(props.position) * 100} h={Position.vmax(props.position) * 0.1} globalAlpha={0.5} fillStyle={props.color} />
