...
function MessageComponent(props) {
  ...
  const cx = contextApp.locationLayout.w - contextApp.unitpx * 0.24 - contextApp.unitpx * 0.08 * (animationCountAppear - animationCountDisappear)
  const cy = contextApp.unitpx * 0.14 + contextApp.unitpx * 0.12 * 1.25 * index

  const { animationCount: animationCountCy } = ReactExtensions.useAnimationDestination({ play: true, defaultCount: cy, destination: cy, rate: contextApp.unitpx * 0.01, postprocess: n => Number(n.toFixed(4)) })
  ...
  const Component =
    <layout cx={cx} cy={animationCountCy} w={contextApp.unitpx * 0.48} h={contextApp.unitpx * 0.12} globalAlpha={globalAlpha}>
