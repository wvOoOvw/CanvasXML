...
function App() {
  ...
  const vmin = Position.vmin(coordinate)
  ...
  layout.forEach((i, index) => ReactAnimation.component(Arc)({ position: Position.center(i), radius: vmin * 4 + Math.cos(animationCount + Math.PI / 2 * (index - 1)) * vmin * 0.4 }))
